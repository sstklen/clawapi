...
## 🩺 内蔵：Dr. Claw — AIデバッグナレッジベース

本リポジトリには [**Dr. Claw**](../drclaw/) が含まれています — 解決した全てのバグを記憶するAIデバッグナレッジベースです。一度聞けば、二度と繰り返しません。

- 6,800以上の問題をインデックス、980以上の検証済み修正
- MCPサーバー + REST API
- Confucius Debugから独立、現在は `clawapi/drclaw/` に格納
