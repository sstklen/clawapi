...
// 使用金鑰匙（gold key）作為 AI 大腦，規劃並執行多步驟任務
...
import type { GoldKeyInfo } from './l3-concierge';
...
export interface TaskStep {
  ...
  id: string;
  /** 使用的工具（對應 Adapter ID，或 "llm_analysis" 代表金鑰匙 LLM 直接分析） */
  tool: string;
...
export interface TaskPlan {
  ...
  /** 預估金鑰匙消耗的 token 數（規劃 + 整合） */
  estimated_gold_key_tokens: number;
  /** 所有任務步驟 */
  steps: TaskStep[];
}
...
export interface CostEstimate {
  ...
  /** 預估金鑰匙 token 消耗 */
  estimated_gold_key_tokens: number;
  /** 是否超過用戶設定的 max_gold_key_tokens 限制 */
  exceeds_limit: boolean;
  /** 用戶設定的限制（0 表示無限制） */
  max_gold_key_tokens: number;
}
...
export interface L4Request {
  ...
  /** 額外參數（可含 max_gold_key_tokens、checkpoint_id 等） */
  params?: Record<string, unknown>;
}
...
特殊工具：
- llm_analysis：使用金鑰匙 LLM 直接分析文字或整合資訊（無需外部 API Key）
...
6. estimated_gold_key_tokens 只計算金鑰匙 LLM 呼叫的 token（規劃 + 整合）
 ...
 * 3. 用金鑰匙 LLM 規劃 DAG 任務計畫
 * 4. 預估成本，若超過 max_gold_key_tokens 則拒絕執行
...
export class L4TaskEngine {
  /** 金鑰匙服務 ID（固定服務 ID，用來選取金鑰匙） */
  private static readonly GOLD_KEY_SERVICE_ID = '__gold_key__';
  ...
  constructor(
    ...
    // 借用 L3Concierge 的金鑰匙管理和 Adapter 輔助方法
  ...
  async execute(req: L4Request): Promise<L4Response> {
    ...
    // === 步驟 1：取得金鑰匙 ===
    const goldKeyResult = await this.getGoldKey();

    // 沒有金鑰匙 → 回傳錯誤 + 建議
    if (!goldKeyResult) {
      return {
        success: false,
        error: '未設定金鑰匙（Gold Key），無法使用 L4 任務引擎功能',
        suggestion: '請執行：clawapi gold-key set <your-api-key> 來設定金鑰匙',
        latency_ms: Date.now() - startTime,
      };
    }

    // 額度檢查：剩餘 5% 以下 → 降級到 L2
    if (this.shouldDegradeToL2(goldKeyResult)) {
      return this.degradeToL2Response(req, startTime);
    }

    // 讀取 max_gold_key_tokens 限制（0 = 無限制）
    const maxGoldKeyTokens = (req.params?.['max_gold_key_tokens'] as number) ?? 0;
    ...
    if (checkpoint) {
      ...
      // 用金鑰匙 LLM 規劃任務
      const planResult = await this.planTask(goldKeyResult.key, req.messages);
      ...
      goldKeyTokens += planResult.tokens ?? 0;
    }

    // === 步驟 4：成本預估 ===
    const costEstimate = this.estimateCost(plan, maxGoldKeyTokens);

    // 若超過 max_gold_key_tokens 限制 → 拒絕執行
    if (costEstimate.exceeds_limit) {
      return {
        success: false,
        error: `任務預估消耗 ${costEstimate.estimated_gold_key_tokens} tokens，超過您設定的上限 ${maxGoldKeyTokens} tokens`,
        suggestion: `請提高 max_gold_key_tokens 參數，或簡化任務描述`,
        cost_estimate: costEstimate,
        goal: plan.goal,
        latency_ms: Date.now() - startTime,
      };
    ...
    // 累計整合階段的金鑰匙 token 消耗
    goldKeyTokens += synthesisResult.tokens ?? 0;
  ...
  /**
   * 取得金鑰匙（Gold Key）
   * 金鑰匙儲存在特殊服務 ID '__gold_key__' 下
   *
   * @returns 金鑰匙資訊，若不存在則回傳 null
   */
  async getGoldKey(): Promise<GoldKeyInfo | null> {
    const key = await this.keyPool.selectKey(L4TaskEngine.GOLD_KEY_SERVICE_ID);
    if (!key) return null;
   ...
   */
  shouldDegradeToL2(goldKeyInfo: GoldKeyInfo): boolean {
    // 若 daily_token_limit = 0 表示無限制，不降級
    if (goldKeyInfo.daily_token_limit === 0) return false;

    const remaining = goldKeyInfo.daily_token_limit - goldKeyInfo.daily_tokens_used;
    const remainingRatio = remaining / goldKeyInfo.daily_token_limit;
  ...
  private async degradeToL2Response(
    ...
    return {
      success: false,
      error: '金鑰匙今日額度不足，降級到 L2 後仍失敗',
      suggestion: '金鑰匙今日 token 額度已接近上限，將在明日重置',
      latency_ms: Date.now() - startTime,
    };
  }
  ...
  async planTask(
    ...
    // 呼叫金鑰匙 LLM
    const llmResult = await this.l3Helper.callLLMWithGoldKey(
      goldKey,
      systemPrompt,
      messages
    );
  ...
  parsePlan(rawContent: string): TaskPlan | null {
    ...
    if (typeof planObj['estimated_gold_key_tokens'] !== 'number') return null;
    ...
    return {
      ...
      estimated_gold_key_tokens: planObj['estimated_gold_key_tokens'],
      steps,
    };
  }
   ...
   *
   * @param plan 任務計畫
   * @param maxGoldKeyTokens 用戶設定的 token 上限（0 = 無限制）
   * @returns 成本預估結果
   */
  estimateCost(plan: TaskPlan, maxGoldKeyTokens: number = 0): CostEstimate {
    const exceedsLimit =
      maxGoldKeyTokens > 0 &&
      plan.estimated_gold_key_tokens > maxGoldKeyTokens;

    return {
      estimated_calls: plan.estimated_calls,
      estimated_gold_key_tokens: plan.estimated_gold_key_tokens,
      exceeds_limit: exceedsLimit,
      max_gold_key_tokens: maxGoldKeyTokens,
    };
  }
  ...
  /**
   * 執行 llm_analysis 特殊工具（直接用金鑰匙 LLM）
   *
   * @param stepId 步驟 ID
  ...
  private async executeLlmAnalysis(
    ...
    // 取得金鑰匙
    const goldKeyInfo = await this.getGoldKey();
    if (!goldKeyInfo) {
      return {
        id: stepId,
        tool: 'llm_analysis',
        success: false,
        error: 'llm_analysis 步驟無法執行：金鑰匙不存在',
        latency_ms: Date.now() - startTime,
        retry_count: retryCount,
      };
    }
    ...
    const result = await this.l3Helper.callLLMWithGoldKey(
      goldKeyInfo.key,
