...
// 當 model='ask' 時觸發
// 使用金鑰匙（gold key）作為 AI 大腦，解讀意圖、規劃步驟、整合結果
...
export class L3Concierge {
  ...
  async execute(req: L3Request): Promise<L3Response> {
    ...
    if (!goldKeyResult) {
      return {
        success: false,
        error: '未設定金鑰匙（Gold Key），無法使用 L3 AI 管家功能',
    ...
    const intentRaw = await this.callLLMWithGoldKey(
      goldKeyResult.key,
      systemPrompt,
      req.messages
    );
    ...
    // 累計金鑰匙 token 消耗
    goldKeyTokens += intentRaw.tokens ?? 0;
  ...
  async getGoldKey(): Promise<GoldKeyInfo | null> {
    const key = await this.keyPool.selectKey(L3Concierge.GOLD_KEY_SERVICE_ID);
    if (!key) return null;
...
請根據以上執行結果，給用戶一個完整、清晰的回答。`;
    ...
    const result = await this.callLLMWithGoldKey(
      goldKey,
  ...
  /**
   * 用金鑰匙呼叫 LLM API
   * 透過 Adapter Executor 直接呼叫金鑰匙對應的服務
   *
   * @param goldKey 金鑰匙
   ...
   */
  async callLLMWithGoldKey(
    goldKey: DecryptedKey,
    systemPrompt: string,
    messages: Array<{ role: string; content: string }>
  ): Promise<{ success: boolean; content?: string; tokens?: number; error?: string }> {
    // 找到金鑰匙對應的 Adapter
    // 金鑰匙的服務 ID 是 '__gold_key__'，但實際 Adapter 可能是任何 LLM 服務
    // 嘗試用 gold key 的 service_id 找 Adapter，失敗則嘗試所有 LLM Adapter
    const adapterForGoldKey = this.findAdapterForGoldKey();

    if (!adapterForGoldKey) {
      return {
        success: false,
        error: '找不到金鑰匙對應的 LLM Adapter，請確認已安裝 LLM 類型的 Adapter',
      };
    }

    const { adapter, modelId } = adapterForGoldKey;
   ...
   */
  private findAdapterForGoldKey(): { adapter: AdapterConfig; modelId: string } | null {
    // 優先找 LLM 類型的 Adapter
